...
for run in ${NBRUNS[@]}; do
  RESDIR="/home/minier/watdiv-bgp-50-load/run${run}"
  mkdir -p $RESDIR
  for nb in ${NBCLIENTS[@]}; do
    python3 scripts/run_load.py watDivQueries/ watDivQueries/query_10175.rq $RESDIR /home/minier/watdiv-reference/results/ $nb bgp
