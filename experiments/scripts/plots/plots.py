    ...
    def process(basePath, clients, nbRuns):
        ...
        for nb in clients:
            ...
            for x in range(1, nbRuns + 1):
                # print('{}/run{}/execution_times_{}c.csv'.format(basePath, x, nb))
...
def compute_throughput(basePath, clients, nbRuns=3, tpf=False):
    def mapper(df):
        t = df['time']
        if t >= 120.0:
            return 120.0
        # manually add network latency if not tpf
        if tpf:
            return t
    ...
    # x = compute_avg_value(processor)(basePath, clients, nbRuns)
    # print(basePath, max(x) - min(x))
...
virtuo_inf = compute_throughput('results/watdiv-virtuoso-inf', clients, 2)
...
tpf = compute_throughput('results/watdiv-tpf', clients, 1, True)
